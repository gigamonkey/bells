...
describe('parsePlainTime', () => {
  ...
  describe('AM inference (both interpretations work when previous is morning)', () => {
    it('9:34 after 8:30 → 09:34 (AM preferred), but is ambiguous (21:34 also after 8:30)', () => {
      const prev = pt(8, 30);
      const { time, ambiguous } = parsePlainTime('9:34', prev);
      // Both 09:34 AM and 21:34 PM are after 08:30, so both work → ambiguous
      assert.strictEqual(time.hour, 9); // AM is returned when both work
      assert.strictEqual(time.minute, 34);
      assert.strictEqual(ambiguous, true);
    });

    it('8:24 after 7:26 → 08:24 (AM preferred), ambiguous (20:24 also works)', () => {
      const prev = pt(7, 26);
    ...
    it('truly unambiguous AM: time > 12 PM of previous, only AM fits — e.g. 14:00 previous, 14:30 next', () => {
      // After 14:00, "2:45" → AM=2:45 (not after 14:00), PM=14:45 (after 14:00) → only PM works
      // For a truly unambiguous AM case we need previous > 12:00 and target hour < prev-12
      // e.g. previous = 13:00, target = '12:30' → AM=12:30 < 13:00, PM=00:30 < 13:00 → neither
      // There's no common case where only AM works with morning schedules.
      // The algorithm: only AM works when pmMs <= prevMs but amMs > prevMs.
      // E.g. previous = 21:00, target = '10:00' → AM=10:00 < 21:00 (no), PM=22:00 > 21:00 (yes) → only PM
      // For only AM: prev=11:00, target='11:30' → AM=11:30 > 11:00 (yes), PM=23:30 > 11:00 (yes) → ambiguous
      // In practice, only AM is unambiguous when prev >= 13:00 and (h+12) <= prev
      // e.g. prev=14:00, h=1 → AM=1:00 < 14:00 (no), PM=13:00 < 14:00 (no) → neither (ambiguous)
      // So truly "only AM works" is rare. Skip this edge case.
      assert.ok(true, 'no common purely-AM-unambiguous case in morning schedules');
    });
  });
  ...
  describe('ambiguous: both interpretations work', () => {
    it('6:30 after 5:00 → ambiguous (both 06:30 and 18:30 are after 05:00)', () => {
      const prev = pt(5, 0);
      const { ambiguous } = parsePlainTime('6:30', prev);
      assert.strictEqual(ambiguous, true);
    });

    it('returns AM time when both work', () => {
      const prev = pt(5, 0);
      const { time } = parsePlainTime('6:30', prev);
