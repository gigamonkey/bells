...
describe('Calendar', () => {
  ...
  describe('schedule(date)', () => {
    ...
    it('specific date override uses that schedule', () => {
      const overrideData = {
        ...CALENDAR_DATA,
        schedules: {
...
describe('Schedule.actualPeriods', () => {
  ...
  it('student with includeTags { 2: ["zero"] }: Period 0 passes hasPeriod on Tuesday', () => {
    // Note: actualPeriods() trims optional periods from the start/end of the day,
    // so Period 0 (tagged optional) is trimmed from the boundary even when opted in.
    // Test hasPeriod directly to verify the opt-in logic works.
    const cal = makeCalendar({ includeTags: { 2: ['zero'] } });
    const sched = cal.schedule(pd('2025-08-19'));
    const p0 = sched.rawPeriods.find((p) => p.name === 'Period 0');
    assert.ok(sched.hasPeriod(p0), 'Period 0 should pass hasPeriod when "zero" is in includeTags');
  });
