...
describe('validateCalendarData', () => {
  ...
  describe('missing required fields', () => {
    for (const field of ['year', 'id', 'name', 'timezone', 'firstDay', 'lastDay', 'schedules']) {
  ...
  describe('nonClassDays', () => {
    const withNonClass = (entries, extraDates = {}) =>
      withPatch((d) => {
        d.dates = {
          ...Object.fromEntries(Object.keys(entries).map((k) => [k, 'NORMAL'])),
          ...extraDates,
        };
        d.nonClassDays = entries;
      });

    it('all dates valid, in dates map, school days → valid', () => {
      const data = withNonClass({
        '2026-06-01': 'exam',
        '2026-06-04': 'bonus',
      });
      const result = validateCalendarData(data);
    ...
    it('invalid date string → error', () => {
      const data = withNonClass({ 'not-a-date': 'exam' });
      const result = validateCalendarData(data);
      assert.strictEqual(result.valid, false);
      assert.ok(result.errors.some((e) => e.includes('not-a-date')));
    });

    it('out of range → error', () => {
      const data = withPatch((d) => {
        d.nonClassDays = { '2024-01-01': 'exam' };
      });
      const result = validateCalendarData(data);
      assert.strictEqual(result.valid, false);
      assert.ok(result.errors.some((e) => e.includes('2024-01-01')));
    ...
    it('weekend date → error', () => {
      // 2025-09-06 is a Saturday
      const data = withPatch((d) => {
        d.nonClassDays = { '2025-09-06': 'exam' };
      });
      const result = validateCalendarData(data);
      assert.strictEqual(result.valid, false);
      assert.ok(result.errors.some((e) => e.includes('weekend')));
    });

    it('holiday date → error', () => {
      // 2025-09-01 is a holiday in fixture
      const data = withPatch((d) => {
        d.nonClassDays = { '2025-09-01': 'exam' };
      });
      const result = validateCalendarData(data);
      assert.strictEqual(result.valid, false);
      assert.ok(result.errors.some((e) => e.includes('holiday')));
    });

    it('date not in dates map → error', () => {
      const data = withPatch((d) => {
        // 2025-08-19 (Tue) is a school day but not overridden in dates
        d.nonClassDays = { '2025-08-19': 'exam' };
      });
      const result = validateCalendarData(data);
      assert.strictEqual(result.valid, false);
      assert.ok(result.errors.some((e) => e.includes('NORMAL schedule')));
    });

    it('non-string label → error', () => {
      const data = withNonClass({ '2026-06-01': 42 });
      const result = validateCalendarData(data);
      assert.strictEqual(result.valid, false);
      assert.ok(result.errors.some((e) => e.includes('non-empty string')));
    });

    it('empty string label → error', () => {
      const data = withNonClass({ '2026-06-01': '' });
      const result = validateCalendarData(data);
      assert.strictEqual(result.valid, false);
    });

    it('missing nonClassDays → valid', () => {
      const data = withPatch((d) => { delete d.nonClassDays; });
      const result = validateCalendarData(data);
      assert.strictEqual(result.valid, true);
    });
  });
