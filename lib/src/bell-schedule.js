...
class BellSchedule {
  ...
  periodsForDate(instant = Temporal.Now.instant()) {
    ...
    if (cal.isInCalendar(instant)) {
      const today = instant.toZonedDateTime(cal.timezone).toPlainDate();
      if (cal.isSchoolDay(today)) {
        ...
        date = Temporal.Instant.compare(instant, endOfDay) < 0
          ? today
          : cal.nextSchoolDayStart(instant).toZonedDateTime(cal.timezone).toPlainDate();
      } else {
        date = cal.nextSchoolDayStart(instant).toZonedDateTime(cal.timezone).toPlainDate();
      }
    } else {
      date = cal.firstDay;
    }
