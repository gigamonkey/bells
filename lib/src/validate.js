...
const validateYear = (year, index) => {
  ...
  const holidaySet = new Set(year.holidays || []);
  for (const [key, value] of Object.entries(year.nonClassDays || {})) {
    const d = tryParseDate(key);
    if (!d) {
      errors.push(`${label}: nonClassDays key "${key}" is not a valid date`);
      continue;
    }
    if (!inRange(key)) {
      errors.push(`${label}: nonClassDays key "${key}" is outside the calendar year range`);
      continue;
    }
    const dow = d.dayOfWeek;
    if (dow === 6 || dow === 7) {
      errors.push(`${label}: nonClassDays date "${key}" falls on a weekend`);
    } else if (holidaySet.has(key)) {
      errors.push(`${label}: nonClassDays date "${key}" is also a holiday`);
    } else if (!(key in (year.dates || {}))) {
      errors.push(
        `${label}: nonClassDays date "${key}" has no entry in dates (uses NORMAL schedule)`
      );
    }
...
const validateCalendarData = (data) => {
  ...
  const ids = new Set(arr.map((y) => y.id).filter((x) => x !== undefined));
  if (ids.size > 1) {
    errors.push(
      `Calendar array mixes multiple ids: ${[...ids].map((s) => `"${s}"`).join(', ')}`
    );
  }
