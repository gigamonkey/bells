...
class Calendar {
  ...
  schedule(date) {
    ...
    if (d in this.dates && d >= this.firstDay.toString()) {
      ...
      periods = typeof entry === 'string' ? this.#namedSchedule(entry) : entry;
    } else {
      const weekdayName = WEEKDAY_NAMES[date.dayOfWeek];
      const name = this.weekdaySchedules[weekdayName];
      periods = name ? this.#namedSchedule(name) : this.#namedSchedule('NORMAL');
    }
