...
class Calendar {
  ...
  #schoolTimeBetween(start, end) {
    ...
    let totalNs = 0n;
    let cursorDate = cursor.toZonedDateTime(this.timezone).toPlainDate();
   ...
   */
  nextHoliday(instant) {
    let d = instant.toZonedDateTime(this.timezone).toPlainDate().add({ days: 1 });
  ...
  nextSchoolDayStart(instant) {
    const date = instant.toZonedDateTime(this.timezone).toPlainDate();
